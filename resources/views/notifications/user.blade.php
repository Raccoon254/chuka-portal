                                        ...
                                        <section class="flex justify-between shadow items-center border-b border-gray-200 mb-2 {{ $notification->isReadByUser(Auth::id()) ? 'border-orange-600' : '' }}">
                                            <div class="px-4 pt-4 mb-4">
                                                ...
                                                <section class="flex items-center mt-3 gap-3">
                                                    <div class="text-xs italic badge {{ $notification->isReadByUser(Auth::id()) ? 'badge-warning' : 'badge-error' }}">
                                                        {{ $notification->isReadByUser(Auth::id()) ? 'Read' : 'Unread' }}
                                                    </div>
                                                    <div class="text-xs badge text-gray-500">{{ $notification->created_at->diffForHumans() }}</div>
                                                </section>
                                            </div>
