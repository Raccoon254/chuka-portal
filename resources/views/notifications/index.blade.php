<x-app-layout>
    <div class="w-full sm:px-2">
        ...
        <div class="mb-2 sm:mb-6 p-1 sm:p-6 w-full">
            <a class="tooltip float-right" data-tip="Create a new notification"
            ...
            @foreach($notifications as $notification)
                <div class="p-1 sm:p-6 shadow bordered border-base-100 rounded mb-4">
                    ...
                    <div class="mt-4 z-20 w-full justify-end flex gap-4">
                        ...
                        <a class="tooltip z-20" data-tip="View {{ $notification->title }}"
                           href="{{ route('notifications.show', $notification->id) }}">
                            <x-round-button class="z-20">{!! '<i class="fa-solid fa-mountain"></i>' !!}</x-round-button>
                        </a>

                        <a class="tooltip z-20" data-tip="Edit {{ $notification->title }}"
                           href="{{ route('notifications.edit', $notification->id) }}">
                            <x-round-button>{!! '<i class="fa-solid fa-gear"></i>' !!}</x-round-button>
                        </a>

                        <form action="{{ route('notifications.destroy', $notification->id) }}" method="POST"
                              class="tooltip z-20" data-tip="Delete {{ $notification->title }}">
                            @csrf
                            @method('DELETE')
                            <x-round-button class="z-20"
                                            type="submit">{!! '<i class="fa-solid fa-trash"></i>' !!}</x-round-button>
                        </form>
